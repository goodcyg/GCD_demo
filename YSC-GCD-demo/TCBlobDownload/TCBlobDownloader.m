...
- (void)start
{
    ...
    BOOL res=[[self.pathToDownloadDirectory lastPathComponent] containsString:@"."];
    if (![fm fileExistsAtPath:self.pathToDownloadDirectory]&&!res)
    {
...
- (void)connection:(NSURLConnection*)connection didReceiveResponse:(NSURLResponse *)response
{
    ...
    if (!error) {
        ...
        [self.receivedDataBuffer replaceBytesInRange: NSMakeRange(0, self.receivedDataBuffer.length) withBytes:NULL length:0];
...
- (void)connection:(NSURLConnection*)connection didReceiveData:(NSData *)data
{
    ...
    if (self.receivedDataBuffer.length > kBufferSize && [self isExecuting]) {
        ...
        @catch (NSException *exception) {
            printf("[NSFileHandle writeData] reason:%s file:%s line:%d",exception.reason.UTF8String,__FILE__,__LINE__);
        }
        [self.receivedDataBuffer replaceBytesInRange: NSMakeRange(0, self.receivedDataBuffer.length) withBytes:NULL length:0];
        //[self.receivedDataBuffer setData:nil];
    }
...
- (NSString *)pathToFile
{
    ...
    NSDictionary *dict= [fm attributesOfItemAtPath:self.pathToDownloadDirectory error:nil];
    if ([dict objectForKey:NSFileType] == NSFileTypeDirectory)
    {
         return [self.pathToDownloadDirectory stringByAppendingPathComponent:self.fileName];
    }
    return _pathToDownloadDirectory;
}
